 ...
 *    2. 每个元素的父元素必须是它本身减 1
 *    3. 若末项等于首项，则删去末项
 *    4. 否则，找到所有等于 (末项 - 1) 的元素，取其中倒数第二个（从右向左第二）作为坏根；
 *       如果只有一个等于 (末项 - 1) 的元素，则取该元素作为坏根。
 *       然后复制坏部 n 次。
 * ============================================================================
 */

/**
 * 展开 DFSS 序列（标准展开），对外使用统一的函数名 expandNormal
...
export function expandNormal(seq, n) {
    ...
    // ---- DFSS 坏根查找（新规则）：取所有等于 last - 1 的索引，取倒数第二个（若只有一个则取那个） ----
    const targetIndices = [];
    ...
    if (targetIndices.length === 0) {
        // 在合法的 PrSS/DFSS 序列中，last - 1 必定存在，但以防万一
        throw new Error("无效序列：找不到等于最后一个元素减1的元素");
    }
    // targetIndices 是从右向左收集的，所以索引0是最右的，索引1是第二右的
...
export function parse(input) {
    ...
    try {
        ...
        throw new Error(`DFSS 序列解析失败: ${err.message}`);
    }
}
