...
export function compare(seq1, seq2) {
   if (seq1.length === 0) {
      ...
      if (seq2.length === 0) return 1;
         ...
         else return compare(seq1.slice(1), seq2.slice(1));
      }
   }
}

export function display(expr) {
   return '' + expr === 'Infinity' ? 'Limit' : '' + expr;
}
...
register.push({
   ...
   able: Y_limit,
   compare: compare,
   expandNormal: expandNormal,          // 原 FS
   FSalter: (seq, FSterm) => {
      if (!seq.length) return [];
      var datakey = '' + seq;
      if (datakey === 'Infinity') return [1, 1 + FSterm];
